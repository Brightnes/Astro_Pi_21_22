"""
Main astro py MSL program; astrob team.
The program gets magnetic field data, acceleration data, photos.
...
Some machine learning code was edited in attempt to identify photos with wavy clouds;
ML code is imported and the function 'chck_wavy_clouds' inside of it is called in the
'doing_stuff' function.
...
def sun_position(la, lon):
    """ This function computes altitude and azimuth of the sun; it's for clouds height post processing computations"""
    ts = load.timescale()
    t = ts.now()
...
def create_csv(data_file):
    ...
    with open(data_file, 'w', buffering=1) as f:
        writer = csv.writer(f)
        header = ("Date/time", "Loop number", "Pic number", "ISS_shadowing_now", "earth_nightime", "Wavy clouds in precedent photo detected", "Mag field x", "Mag field y","Mag field z", "Acceler x", "Acceler y", "Acceler z", "Latitude", "Longitude", "North direction", "Sun altitude", "Sun azimuth")
...
def doing_stuff(a, b):
    """ This function does all the main work: it saves one data row and a photo; a and b parameters are boolean about shadowing condition of ISS and Earth respectively."""
    ...
    try:
        ...
        row = (datetime.now(), i, pic, a, b, wavy_clouds_in_photo, mag_value["x"], mag_value["y"], mag_value["z"], acceler["x"], acceler["y"], acceler["z"], lat, long, north_direction, sun_altitude, sun_azimuth)# Add entries if needed
